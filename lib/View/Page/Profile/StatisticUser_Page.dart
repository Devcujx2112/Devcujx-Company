...
class _StatisticUserPageState extends State<StatisticUserPage> {
  ...
  void ShowAllDataOrder() async {
    ...
    if (authVM.uid!.isNotEmpty) {
      ...
      Map<String, dynamic>? dataMonth =
          await orderVM.StatisticUser(authVM.uid!);
      if (dataOrder != null && dataMonth != null) {
        setState(() {
          allOrder = soLuong;
          orders = dataOrder;
          nullOrder = false;
          dataNull = false;
          dataStatisticMonth = dataMonth;
  ...
  List<ChartData> get _yearData{
    return List.generate(4, (index){
      final yearKey = index + 1;
      return ChartData(yearKey.toString(), dataStatisticYear[yearKey]);
    });
  }
